...
public class DevanningBizImpl implements DevanningBiz {
	...
	public void devanning(DevanningSubmitRequest request) {
		...
		if (request.getIsSn()) {
			// TODO bug
			// 根据stockId对序列号进行分组
			// 每个stockId调用一次moveStock（一个stockId可能对应多个序列号，如果一个个调用会出现数据库乐观锁更新失败）
			...
			// 库位移动
			stockBiz.moveStock(stock, serialNumberList, stock.getStockBalance(), location,
					StockLogTypeEnum.STOCK_DEVANNING_BY_PDA, null, null, null);
		} else {
			...
			request.getStockList().forEach(stockDeva -> {
				...
				// lpn等同于库位
				stock.setLpnCode(location.getLocCode());
				// TODO bug 移动库存的时候sourceStock不应该变化，此处应该用其它的重名函数
				// 如果用户输入的数量大于0则进行拆箱
				if (BigDecimalUtil.gt(stockDeva.getSplitQty(), BigDecimal.ZERO)) {
					stockBiz.moveStock(stock, null, stockDeva.getSplitQty(), location,
							StockLogTypeEnum.STOCK_DEVANNING_BY_PDA, null, null, null);
				}
			});
		}
