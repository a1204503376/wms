...
public class MergeFunctionCode extends FunctionCodeBase implements IFunctionCode {
	...
	public void execute(Stock stock, InstockDetail instockDetail, InstockExecuteVO instockExecute) {
		...
		String msg = String.format("上架策略[%s]的第[%s]个策略执行成功, 推荐从库区[%s]上架到库区[%s]",
			instock.getSsiName(), instockDetail.getSsidProcOrder(),
			sourceZone.getZoneName(), NodesUtil.join(targetZoneList, "zoneName"));
		if (Func.isNotEmpty(targetLoc)) {
