...
public class MissionWrapper extends BaseEntityWrapper<Mission, MissionVO> {
	...
	public MissionVO entityVO(Mission mission) {
		...
		//任务类型
		missionVO.setMissionTypeName(DictCache.getValue(DictCodeConstant.TASK_TYPE, mission.getMissionType()));
		//推送方式
		missionVO.setPushWayName(DictCache.getValue(DictCodeConstant.TASK_PUSH_WAY, mission.getPushWay()));
		//计算方式
		missionVO.setCountWayName(DictCache.getValue(DictCodeConstant.TASK_COUNT_WAY, mission.getCountWay()));
		//超过数量是否分割
