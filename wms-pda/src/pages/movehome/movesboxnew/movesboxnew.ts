...
@IonicPage()
@Component({
  selector: 'page-movesboxnew',
  templateUrl: 'movesboxnew.html',
})
export class MovesBoxNewPage {
  /**
    * 页面元素
    */
  ...
  boxCode_KeyDown(event) {
    if (event && event.keyCode == 13) {
      ...
      this.getInfoMoveByBoxCode();
      // this.boxItems.push({boxCode:this.boxCode,skuCode:'1234',skuName:'4321',reZone:'Z01-01-01',reLocCode:'A01-01-01',isChecked:true})
    }
  }
  
  getInfoMoveByBoxCode(){
    this.nativeService.showLoading();
    this.appService.httpRequest(Api.Instock + Api.getInfoMoveByBoxCode, Method.Get, {boxCode:this.boxCode}, '', result => {
      this.nativeService.hideLoading();
      if(result.success){
...
*/
  ...
  */
  ...
  btnOk() {
    ...
    this.nativeService.showLoading();
    this.appService.httpRequest(Api.Instock + Api.submitMoveForBoxNew, Method.Post, param, ContenType.Json, result => {
