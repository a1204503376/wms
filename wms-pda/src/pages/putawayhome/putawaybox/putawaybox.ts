...
@IonicPage()
@Component({
  selector: 'page-putawaybox',
  templateUrl: 'putawaybox.html',
})
export class PutawayboxPage {
  /**
    * 页面元素
    */
...
*/
  ...
  */
  ...
  btnOk() {
    debugger
    if (Utils.isEmpty(this.stockId)) {
      this.nativeService.showToast('没有查询到库存信息');
      return;
    }
    if (Utils.isEmpty(this.scanModel)) {
    ...
    this.nativeService.showLoading();
    this.appService.httpRequest(Api.Instock + Api.submitPutawayForBox, Method.Post, param, ContenType.Json, result => {
 ...
 */
  ...
  clearModel() {
    this.skuLotModel = new SkuLotModel();//批属性对象
