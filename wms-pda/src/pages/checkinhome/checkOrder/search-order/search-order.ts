...
export class SearchOrderPage {
  ...
  */
  ...
  headerInfoForOrder() {
    ...
    this.appService.httpRequest(Api.Instock + Api.getAsnHeaderAndDetails, Method.Get, { asnBillNo: this.asnBillNo }, '', result => {
      this.nativeService.hideLoading();
      if (result.success) {
        if (Utils.isEmpty(result.data)) {
          this.nativeService.showToast('该单据已经完成或不存在');
        } else {
            this.navCtrl.push('CheckOrderPage', { result: result.data });
        }
      } else {
