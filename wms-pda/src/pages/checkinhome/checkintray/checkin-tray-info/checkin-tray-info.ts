...
@IonicPage()
@Component({
  selector: 'page-checkin-tray-info',
  templateUrl: 'checkin-tray-info.html',
})

export class CheckInTrayInfoPage {
  ...
  lpnCode_KeyDown(event) {
    if (event && event.keyCode == 13) {
      if(this.lpnItems.find((n)=>n==this.lpnCode)){
        this.nativeService.showToast("该托盘已扫描过！");
        return;
      }
      this.lpnItems.push(this.lpnCode);
    }
  }
  ionViewWillLeave() {
 ...
 */
  ...
  btnOk() {
    ...
    let params = {
      ...
      lpnCodes: this.lpnItems
      // lpnCode: " ",
      // taskId: this.taskId
    };
    this.nativeService.showLoading();
    this.appService.httpRequest(Api.Instock + Api.submitAsnHeaderForTray, Method.Post, params, ContenType.Json, result => {
      this.nativeService.hideLoading();
