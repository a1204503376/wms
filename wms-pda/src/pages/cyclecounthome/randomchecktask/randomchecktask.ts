...
@IonicPage()
@Component({
    selector: 'page-randomchecktask',
    templateUrl: 'randomchecktask.html',
})
export class RandomCheckTaskPage {
    ...
    openSkuSelectModal(data1) {
        let myModal = this.modalCtrl.create('SkuSelectModal', {
            skuRecords: data1,
        });
        myModal.onDidDismiss(data => {
            if (Utils.isEmpty(data)) return;
            this.scanModel.skucode = data.skuCode;
        });
        myModal.present();
    }
    ...
    querySkus() {
        debugger
        if (this.result && this.result.mode == '0') {
            let objs = [];
            for (let item of this.result.countSkuDetails) {
                let obj = {};
                obj['skuCode'] = item.skuCode;
                obj['mode'] = this.result.mode;
                objs.push(obj);
            }
            this.openSkuSelectModal(objs);
        } else {
            this.openSkuSelectModal(this.result.skuQtyVOS);
        }
    }
        ...
        if (event && event.keyCode == 13) {
            if (Utils.isNotEmpty(this.scanView)) {
                this.barcodeService.ScanSkuBarcode(this.scanView).subscribe(val => {
                    ...
                    this.getSkuBaseUm(val ? val : this.scanView);
                    // }
                })
            }
        }
    ...
    addToList(val: ScanModel) {
        if (this.countBy == 0) {
            let value = this.result.countSkuDetails.filter(x => x.skuCode == val['skucode']);
            console.log(this.result.countSkuDetails);
            console.log(value + "," + JSON.stringify(val));
            if (Utils.isEmpty(value)) {
                this.nativeService.showToast("请盘点指定物品！");
                return;
            }
        }
        this.scanModel = val;
    ...
    submitRandomCheck() {
        ...
        console.log(params);
        this.nativeService.showLoading();
        this.appService.httpRequest(Api.StockCountPDA + Api.randomCheckTaskSubmit, Method.Post, params, ContenType.Json, result => {
            this.nativeService.hideLoading();
            if (result.code == 200) {
