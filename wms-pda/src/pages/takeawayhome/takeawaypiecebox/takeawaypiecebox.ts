...
@IonicPage()
@Component({
  selector: 'page-takeawaypiecebox',
  templateUrl: 'takeawaypiecebox.html',
})
export class TakeawaypieceboxPage {
  ...
  getHeadInfo(wellenNo) {
    ...
    this.appService.httpRequest(Api.Outstock + Api.getPickInfoByBox, Method.Get, params, ContenType.Form, result => {
      ...
      if (Utils.isNotEmpty(this.pickPlansModel)) {
        this.navCtrl.push('TakeawayinboxPage', { pickPlansModel: this.pickPlansModel, PickInfoResult: this.PickInfoResult, pickPlansList: this.pickPlansList, wellenNo: wellenNo });
      }
    });
  }
}
