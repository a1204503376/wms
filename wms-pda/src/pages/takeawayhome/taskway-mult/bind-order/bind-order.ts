...
@IonicPage()
@Component({
  selector: 'page-bind-order',
  templateUrl: 'bind-order.html',
})
export class BindOrderPage {
  ...
  addToList(val:ScanModel){
    ...
    if(Utils.isEmpty(this.scanModelRecords)){
     ...
     for(let item of this.scanModelRecords){
        if(Utils.isObjectValueEqual1(this.scanModel,item,['skuname','skulots','selected','isCheck','qty','setLotModel','setLotValue'])){
           item.qty=Number.parseInt(item.qty)+Number.parseInt(this.scanModel.qty);
           item['selected']=true;
           this.scanModel=item;
        }else{
         if(!Utils.contains(this.scanModelRecords,this.scanModel,['skuname','skulots','selected','isCheck','qty','setLotModel','setLotValue'])){
           this.scanModelRecords.push(this.scanModel);
           break;
         }
        }
