...
@IonicPage()
@Component({
  selector: 'page-takeawaysearch',
  templateUrl: 'takeawaysearch.html',
})
export class TakeawaySearchPage {
  ...
  getHeadInfo(wellenNo) {
    ...
    this.appService.httpRequest(Api.Outstock + Api.getPickInfoByBox, Method.Get, params, ContenType.Form, result => {
      ...
      if (Utils.isNotEmpty(this.pickPlansModel)) {
        this.navCtrl.push('TakeawayinPage', { pickPlansModel: this.pickPlansModel, PickInfoResult: this.PickInfoResult, pickPlansList: this.pickPlansList, wellenNo: wellenNo });
      }
    });
  }
}
