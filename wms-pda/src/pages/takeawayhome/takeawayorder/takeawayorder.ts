...
@IonicPage()
@Component({
  selector: 'page-takeawayorder',
  templateUrl: 'takeawayorder.html',
})
export class TakeawayOrderPage {
  ...
  onClick(record){
    this.getHeadInfo(record.billNo,record.soBillNo);
  }
  getContainerList() {
    ...
    this.appService.httpRequest(Api.api + Api.takewayList, Method.Post, 
      ...
      if(result.data.records.length==1){
        this.getHeadInfo(result.data.records[0].billNo,result.data.records[0].soBillNo);
      }else{
  ...
  */
  ...
  getHeadInfo(wellenNo,soBillNo) {
    this.navCtrl.push('TakeawayinOrderPage',{wellenNo:wellenNo,soBillNo:soBillNo});
    // let params = {
    //   wellenNo: wellenNo
    // }
    ...
    //     this.navCtrl.push('TakeawayinOrderPage', { pickPlansModel: this.pickPlansModel, PickInfoResult: this.PickInfoResult, pickPlansList: this.pickPlansList, wellenNo: wellenNo });
    //   }
    // });
  }
}
